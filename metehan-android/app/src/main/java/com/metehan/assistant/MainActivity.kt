...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        box.addView(TextView(this).apply { text = "Kişisel Yapay Zekâ Başdanışman · Native V0.5"; textSize = 16f })
        status = TextView(this).apply { textSize = 17f; setPadding(0, 28, 0, 28) }
        box.addView(status)

        box.addView(button("⚡ Native Komuta Merkezi") { startActivity(Intent(this, CommandCenterActivity::class.java)) })
        box.addView(button("1 · Mikrofon ve kamera izinleri") {
        ...
        box.addView(button("Canlı Metehan ses panelini aç") { startActivity(Intent(this, MetehanPanelActivity::class.java)) })
        ...
        val url = TextInputEditText(this).apply {
            setText(CorePrefs.coreUrl(this@MainActivity))
            hint = "https://... veya http://127.0.0.1:8765"
        }
        val token = TextInputEditText(this).apply {
            setText(CorePrefs.accessToken(this@MainActivity))
            hint = "Erişim anahtarı (opsiyonel)"
        }
        box.addView(url)
        box.addView(token)
        box.addView(button("Bağlantıyı kaydet ve test et") {
            try {
                ...
                Thread {
                    val msg = runCatching { CoreClient(this).health() }
                        .fold({ "Çekirdek hazır: $it" }, { "Bağlantı hatası: ${it.message}" })
                    runOnUiThread {
        ...
        box.addView(button("Android ses giriş ayarlarını aç") {
    ...
    private fun toggleWakeTest() {
        ...
        testWake = WakeWordController(
            this,
            { Toast.makeText(this, "Uyandı: $it", Toast.LENGTH_LONG).show() },
    ...
    private fun updateStatus() {
        ...
        status.text = "Mikrofon: ${if (mic) "✓" else "—"}   Kamera: ${if (cam) "✓" else "—"}\n" +
            ...
            "Native Agent: V0.5 güvenli eylem kapısı aktif"
    }
