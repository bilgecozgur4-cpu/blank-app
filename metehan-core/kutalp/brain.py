...
CORE_IDENTITY = """You are METEHAN, a personal AI chief adviser.
...
RED_TEAM_PROMPT = """Act as METEHAN's independent Red Team. Critique the proposed answer aggressively but fairly. Look for unsupported assumptions, confirmation bias, hidden costs, missing evidence, false precision and safer/better alternatives. Return only the critique and what would change the conclusion."""
...
def _call_model(instructions: str, user_input: str, timeout: float = 90.0) -> str:
    ...
    user_id = os.getenv("METEHAN_USER_ID", os.getenv("KUTALP_USER_ID", "local-owner")).encode("utf-8")
    payload = {"model": model, "instructions": instructions, "input": user_input, "safety_identifier": hashlib.sha256(user_id).hexdigest()}
...
def offline_response(user_input: str, memories: Iterable[Memory]) -> str:
    ...
    return f"METEHAN çekirdeği çalışıyor fakat henüz model API anahtarı bağlı değil. Yerel hafızadan {memory_count} ilgili kayıt buldum."
